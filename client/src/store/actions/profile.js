...
export const getStudentProfile = () => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch({ type: GET_ASSIGNMENTS, payload: student._assignments });
    }
  } catch (error) {
...
export const getTeacherProfile = () => async dispatch => {
  try {
    ...
    if (response.ok) {
      const users = teacher._assignment._students.map(student => student._user);
      dispatch({ type: GET_TEACHER_PROFILE, payload: teacher });
      dispatch({ type: GET_GRADES, payload: teacher._grades });
      dispatch({ type: GET_ASSIGNMENTS, payload: [teacher._assignment] });
